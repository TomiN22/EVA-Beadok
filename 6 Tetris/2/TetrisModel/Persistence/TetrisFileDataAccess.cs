    ...
    public class TetrisFileDataAccess : ITetrisDataAccess
    {
        public async Task<Table> LoadAsync(String path)
        {
            try
            {
                using (StreamReader reader = new StreamReader(path)) // fájl megnyitása
                {
                    ...
                    Int32 tableSizeX = Int32.Parse(numbers[0]); // beolvassuk a tábla 
                    Int32 tableSizeY = Int32.Parse(numbers[1]);
                    Table table = new Table(tableSizeX,tableSizeY); // létrehozzuk a táblát
                    ...
                    for (Int32 j = 0; j < table.GetSizeX; j++)
                    {
                        line = reader.ReadLine() ?? String.Empty;
                        numbers = line.Split(' ');

                        for (Int32 i = 0; i < table.GetSizeY; i++)
                        {
        ...
        public async Task SaveAsync(String path, Table table)
        {
            try
            {
                ...
                {
                    await writer.WriteLineAsync($"{table.GetSizeX} {table.PCurrent[1]} {table.PCurrent[0]} {table.GameTime} {table.StdTime} {table.FastTime}");
                    for (Int32 j = 0; j < table.GetSizeX; j++)
                    {
                        for (Int32 i = 0; i < table.GetSizeY; i++)
                        {
