...
using static FenymotorModel.Model.GameModel;
using static FenymotorModel.Persistence.Table;
using static System.Net.Mime.MediaTypeNames;

namespace FenymotorWPF.ViewModel
{
    public class FViewModel : ViewModelBase
    {
        ...
        public FViewModel(GameModel model)
        {
        ...
        private void RefreshTable()
        {
            ...
            foreach (Field field in Fields)
            {
                ...
                if (_model.Table.GetTableValue(field.X, field.Y) == 11)
                {
                    field.Colour = "SkyBlue"; OnPropertyChanged("Colour");
                }
                if (_model.Table.GetTableValue(field.X, field.Y) == 22)
                {
                    field.Colour = "Salmon"; OnPropertyChanged("Colour");
                }

                //field.Text = _model.Table.GetTableValue(field.X, field.Y).ToString();
            }

            OnPropertyChanged(nameof(EggsEaten));
