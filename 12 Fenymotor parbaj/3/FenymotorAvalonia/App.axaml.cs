...
public partial class App : Application
{
            ...
            /*desktop.Startup += async (s, e) =>
            {
                ...
                try
                {
                    await _model.LoadGameAsync(
                        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "FenymotorSuspendedGame"));
    ...
    private async void ViewModel_LoadGame(object? sender, System.EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Title = "Fenymotor tábla betöltése",
                AllowMultiple = false,
                FileTypeFilter = new[]
                {
                    new FilePickerFileType("Fenymotor tábla")
                    {
                        Patterns = new[] { "*.stl" }
                    }
                }
    ...
    private async void Model_GameOver(object? sender, FenymotorEventArgs e)
    {
        _timer.Stop();
