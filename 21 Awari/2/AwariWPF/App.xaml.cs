...
namespace AwariWPF
{
    ...
    public partial class App : Application
    {
        ...
        private void App_Startup(object? sender, StartupEventArgs e)
        {
            ...
            _viewModel.FourGame += new EventHandler(ViewModel_4Game);
            _viewModel.EightGame += new EventHandler(ViewModel_8Game);
            _viewModel.TwelveGame += new EventHandler(ViewModel_12Game);
        ...
        private void ViewModel_4Game(object? sender, EventArgs e)
        {
            if (_model != null)
            {
        ...
        private void ViewModel_8Game(object? sender, EventArgs e)
        {
            if (_model != null)
            {
                _model.GameOver -= Model_GameOver;
            }
            _model = new GameModel(_dataAccess, 6);
            _viewModel.Model = _model;
            _viewModel.CreateTable();
            _model.GameOver += Model_GameOver;
            _timer.Start();
        ...
        private void ViewModel_12Game(object? sender, EventArgs e)
        {
            if (_model != null)
            {
                _model.GameOver -= Model_GameOver;
            }
            _model = new GameModel(_dataAccess, 8);
            _viewModel.Model = _model;
            _viewModel.CreateTable();
            _model.GameOver += Model_GameOver;
            _timer.Start();
