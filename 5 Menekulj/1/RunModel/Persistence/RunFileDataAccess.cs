    ...
    public class RunFileDataAccess : IRunDataAccess
    {
        public async Task<Table> LoadAsync(String path)
        {
            try
            {
                using (StreamReader reader = new StreamReader(path)) // fájl megnyitása
                {
                    ...
                    table.PCurrent[0] = Int32.Parse(numbers[1]);
                    table.PCurrent[1] = Int32.Parse(numbers[2]);
                    table.E1Current[0] = Int32.Parse(numbers[3]);
                    table.E1Current[1] = Int32.Parse(numbers[4]);
                    table.E2Current[0] = int.Parse(numbers[5]);
                    table.E2Current[1] = int.Parse(numbers[6]);
        ...
        public async Task SaveAsync(String path, Table table)
        {
            try
            {
                ...
                {
                    await writer.WriteLineAsync($"{table.GetSize} {table.PCurrent[0]} {table.PCurrent[1]} {table.E1Current[0]} {table.E1Current[1]} {table.E2Current[0]} {table.E2Current[1]}");
                    for (Int32 j = 0; j < table.GetSize; j++)
                    {
                        for (Int32 i = 0; i < table.GetSize; i++)
                        {
