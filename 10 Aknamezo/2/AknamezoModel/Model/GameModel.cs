    ...
    public class GameModel
    {
        ...
        public void GenerateShips()
        {
            ...
            for (int i = 0; i < shipCount; i++)
            {
                ...
                Ship ship = new Ship(0, position);
                ship.ShipStepEvent += OnShipStep;
                Ships.Add(ship);
                Table.SetTableValue(0, position, 3);
            }
        }
        ...
        public void GenerateMeteors()
        {
            for (int i = 0; i < GetSize; i++)
            {
                if(_table.GetTableValue(0,i) == 3)
                {
                    Meteor meteor = new Meteor(1, i);
                    meteor.MeteorStepEvent += OnMeteorStep;
                    Meteors.Add(meteor);
                    if(meteor.BombSize == BombSizes.Small)
                    {
                        Table.SetTableValue(1, i, 2);
                    }
                    else if (meteor.BombSize == BombSizes.Medium)
                    {
                        Table.SetTableValue(1, i, 4);
                    }
                    else if (meteor.BombSize == BombSizes.Big)
                    {
                        Table.SetTableValue(1, i, 5);
                    }
                }
            }
        }
        ...
        public void TakeDownAsteroids()
        {
            for (int i = 0; i < GetSize; i++)
            {
                if (_table.GetTableValue(GetSize-1,i) == 2 || _table.GetTableValue(GetSize-1, i) == 4 || _table.GetTableValue(GetSize-1, i) == 5)
                {
        ...
        {
            if (x >= 1 && x < GetSize && y >= 0 && y < GetSize)
            {
        ...
        public void OnShipStep(object? sender, ShipStepEventArgs s)
        {
            if (sender is Ship ship)
            {
                ...
                if (toY < 0 || toY >= GetSize || _table.GetTableValue(toX,toY) != 0)
                {
