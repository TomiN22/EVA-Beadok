...
namespace AknamezoWinForms
{
    public partial class Form : System.Windows.Forms.Form
    {
        ...
        private AknamezoFileDataAccess _dataAccess;
        ...
        private void GameOver(Object? sender, AknamezoEventArgs e)
        {
            timer1.Stop();
            timer2.Stop();
            timer3.Stop();
            MessageBox.Show(PlayerTime.ToString(), "Game over!, You've been blown up!");
        ...
        public void RefreshTable()
        {
            for (int i = 0; i < _model.GetSize; i++)
            {
                for (int j = 0; j < _model.GetSize; j++)
                {
                    ...
                    if (_model.Table.GetTableValue(i, j) == 5)
                    {
                        _buttonGrid[i, j].BackColor  = Color.Red;
                    }
                }
            }
            TimeLabel.Text = StdGameTime.ToString();
            //GasLabel.Text = _model.Table.CurrentGas.ToString();
            EnableButtons = true;
            StartPause=true;
        }
